...
The goal of `{ouch}` is to (try) to provide advices on obscure R error messages. 

## Disclaimer

The messages returned by `{ouch}` try to explain what the error is. There is no guarantee they succeed. 

So far, there are `r nrow(read.csv("inst/en_errors.csv"))` errors analysed.

Also, please note that this package will override any other error handler running, until you call `stop_ouch()` (but I'm working on that).
...
Please note that the 'ouch' project is released with a [Contributor Code of Conduct](CODE_OF_CONDUCT.md). By contributing to this project, you agree to abide by its terms.
